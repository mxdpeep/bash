...
do
  ...
  r=$(curl -s -X POST "${CF_API}/zones/${id}/dns_records" \
    -H "Authorization: Bearer ${CF_TOKEN}" -H "Content-Type: application/json" \
    --data '{"type":"A","name":"'${sub}'","content":"'${content}'","ttl":1,"proxied":false}')
  result=$(echo $r|jq ".success")

  # check result
  if [ "$result" == "true" ]; then
    infogreen "$j/$count Zone: ${name}"
    info "added A record: ${sub} ${content}"
    echo "Zone id: ${id} Account: ${owner}"
  else
    infored "$j/$count Zone: ${name}"
    echo "Zone id: ${id} Account: ${owner}"
