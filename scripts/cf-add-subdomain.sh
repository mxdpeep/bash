...
# parameters
if [ $# -lt 2 ]; then info "\nUsage: \t$0 <all|domain.tld> <subdomain> [<value>]\n\n"; exit; fi
if [ $# -eq 2 ]; then dom=$1; sub=$2; val=""; fi
if [ $# -eq 3 ]; then dom=$1; sub=$2; val=$3; fi
if [ $# -gt 3 ]; then info "\nUsage: \t$0 <all|domain.tld> <subdomain> [<value>]\n\n"; exit; fi
...
do
  ...
  # add CNAME record
  if [ "$val" == "" ]; then content=${name}; else content=${val}; fi
  r=$(curl -s -X POST "${CF_API}/zones/${id}/dns_records" \
    -H "Authorization: Bearer ${CF_TOKEN}" -H "Content-Type: application/json" \
    --data '{"type":"CNAME","name":"'${sub}'","content":"'${content}'","ttl":1,"proxied":true}')
  ...
  if [ "$result" == "true" ]; then
    echo -en "$j/$count "
    infogreen "${name}"
    info "added CNAME record: ${sub} > ${content}"
